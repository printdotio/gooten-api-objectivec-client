...
NSString *const GResponseObjectErrorKey = @"GResponseObject";
...
@implementation GApiClient

- (instancetype)init {
    NSString *baseUrl = [[GConfiguration sharedConfig] host];
...
+(NSNumber*) nextRequestId {
    @synchronized(self) {
        ...
        GDebugLog(@"got id %ld", nextId);
        return [NSNumber numberWithLong:nextId];
    }
}

+(NSNumber*) queueRequest {
    NSNumber* requestId = [GApiClient nextRequestId];
    GDebugLog(@"added %@ to request queue", requestId);
...
-(Boolean) executeRequestWithId:(NSNumber*) requestId {
    ...
    if (matchingItems.count == 1) {
        GDebugLog(@"removed request id %@", requestId);
        [queuedRequests removeObject:requestId];
        return YES;
    }
    else {
    ...
    [self.reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status) {
            case AFNetworkReachabilityStatusUnknown:
                GDebugLog(@"reachability changed to AFNetworkReachabilityStatusUnknown");
                [GApiClient setOfflineState:true];
                break;

            case AFNetworkReachabilityStatusNotReachable:
                GDebugLog(@"reachability changed to AFNetworkReachabilityStatusNotReachable");
                [GApiClient setOfflineState:true];
                break;

            case AFNetworkReachabilityStatusReachableViaWWAN:
                GDebugLog(@"reachability changed to AFNetworkReachabilityStatusReachableViaWWAN");
                [GApiClient setOfflineState:false];
                break;

            case AFNetworkReachabilityStatusReachableViaWiFi:
                GDebugLog(@"reachability changed to AFNetworkReachabilityStatusReachableViaWiFi");
                [GApiClient setOfflineState:false];
                break;
            default:
                break;
                                                               ...
                                                               success:^(AFHTTPRequestOperation *operation, id response) {
                                                                   ...
                                                                   if ([self executeRequestWithId:requestId]) {
                                                                       ...
                                                                       if (operation.responseObject) {
                                                                           ...
                                                                           userInfo[GResponseObjectErrorKey] = operation.responseObject;
                                                                       }
...
-(NSNumber*) requestWithPath: (NSString*) path
    ...
    if ([requestContentType isEqualToString:@"application/json"]) {
        self.requestSerializer = [GJSONRequestSerializer serializer];
    }
    ...
    if ([responseContentType isEqualToString:@"application/json"]) {
        self.responseSerializer = [GJSONResponseSerializer serializer];
    }
    else {
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        [resourcePath replaceCharactersInRange:[resourcePath rangeOfString:[NSString stringWithFormat:@"%@%@%@", @"{", key, @"}"]]
                                    withString:[GApiClient escape:obj]];
    }];
    ...
    if (offlineState) {
        GDebugLog(@"%@ cache forced", resourcePath);
        [request setCachePolicy:NSURLRequestReturnCacheDataDontLoad];
    }
    else if(!hasHeaderParams && [method isEqualToString:@"GET"] && cacheEnabled) {
        GDebugLog(@"%@ cache enabled", resourcePath);
        [request setCachePolicy:NSURLRequestUseProtocolCachePolicy];
    }
    else {
        GDebugLog(@"%@ cache disabled", resourcePath);
    ...
    NSNumber* requestId = [GApiClient queueRequest];
        ...
        for(NSString * key in [queryParams keyEnumerator]){
            ...
            if ([queryParam isKindOfClass:[NSString class]]){
                [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                                          [GApiClient escape:key], [GApiClient escape:[queryParams valueForKey:key]]]];
            }
            else if ([queryParam isKindOfClass:[GQueryParamCollection class]]){
                GQueryParamCollection * coll = (GQueryParamCollection*) queryParam;
                NSArray* values = [coll values];
                NSString* format = [coll format];

                if ([format isEqualToString:@"csv"]) {
                    [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                        [GApiClient escape:key], [NSString stringWithFormat:@"%@", [values componentsJoinedByString:@","]]]];
                    ...
                    for(id obj in values) {
                        [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                            [GApiClient escape:key], [NSString stringWithFormat:@"%@", obj]]];
                        counter += 1;
                    }
                ...
                [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator,
                                          [GApiClient escape:key], [NSString stringWithFormat:@"%@", [queryParams valueForKey:key]]]];
            }
...
- (id) sanitizeForSerialization:(id) object {
    ...
    else if ([object isKindOfClass:[NSString class]] || [object isKindOfClass:[NSNumber class]] || [object isKindOfClass:[GQueryParamCollection class]]) {
        return object;
    }
    ...
    else if ([object isKindOfClass:[GObject class]]) {
